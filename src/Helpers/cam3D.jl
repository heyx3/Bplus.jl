...
cam_projection_mat(cam::Cam3D{F})::Mat{4, 4, F} where {F} = cam_projection_mat(cam.projection)
cam_projection_mat(p::OrthographicProjection{F}) = m4_ortho(p)
cam_projection_mat(p::PerspectiveProjection{F}) = m4_projection(
