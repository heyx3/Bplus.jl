...
using namespace EASY_BMP_NAMESPACE;


bool EasyBMPwarnings = false;

void EASY_BMP_NAMESPACE::SetEasyBMPwarningsOff( void )
{ EasyBMPwarnings = false; }
void EASY_BMP_NAMESPACE::SetEasyBMPwarningsOn( void )
{ EasyBMPwarnings = true; }
bool EASY_BMP_NAMESPACE::GetEasyBMPwarningState( void )
{ return EasyBMPwarnings; }

/* These functions are defined in EasyBMP_DataStructures.h */

int EASY_BMP_NAMESPACE::IntPow( int base, int exponent )
{
 int i;
 int output = 1;
...
void EASY_BMP_NAMESPACE::BMFH::SwitchEndianess( void )
{
...
EASY_BMP_NAMESPACE::BMIH::BMIH()
{
 biPlanes = 1;
 biCompression = 0;
 biXPelsPerMeter = EBMP_DefaultXPelsPerMeter;  
 biYPelsPerMeter = DefaultYPelsPerMeter;
 biClrUsed = 0;
 biClrImportant = 0;
}

void EASY_BMP_NAMESPACE::BMIH::SwitchEndianess( void )
{
...
void EASY_BMP_NAMESPACE::BMIH::display( void )
{
 using namespace std;
...
void EASY_BMP_NAMESPACE::BMFH::display( void )
{
 using namespace std;
...
RGBApixel EASY_BMP_NAMESPACE::BMP::GetPixel(int i, int j) const
{
    using namespace std;
    bool Warn = false;
    if (i >= Width)
...
bool EASY_BMP_NAMESPACE::BMP::SetPixel( int i, int j, RGBApixel NewPixel )
{
 Pixels[i][j] = NewPixel;
 return true;
}
...
bool EASY_BMP_NAMESPACE::BMP::SetColor( int ColorNumber , RGBApixel NewColor )
{
 using namespace std;
...
RGBApixel EASY_BMP_NAMESPACE::BMP::GetColor( int ColorNumber )
{ 
 RGBApixel Output;
 Output.Red   = 255;
 Output.Green = 255;
...
EASY_BMP_NAMESPACE::BMP::BMP( BMP& Input )
{
...
RGBApixel* EASY_BMP_NAMESPACE::BMP::operator()(int i, int j)
{
 using namespace std;
 bool Warn = false;
 if( i >= Width )
...
int EASY_BMP_NAMESPACE::BMP::TellBitDepth( void )
{ return BitDepth; }

// int BMP::TellHeight( void ) const
int EASY_BMP_NAMESPACE::BMP::TellHeight( void )
{ return Height; }

// int BMP::TellWidth( void ) const
int EASY_BMP_NAMESPACE::BMP::TellWidth( void )
{ return Width; }

// int BMP::TellNumberOfColors( void ) const
int EASY_BMP_NAMESPACE::BMP::TellNumberOfColors( void )
{
...
bool EASY_BMP_NAMESPACE::BMP::SetBitDepth( int NewDepth )
{
 using namespace std;
...
bool EASY_BMP_NAMESPACE::BMP::SetSize(int NewWidth , int NewHeight )
{
 using namespace std;
...
bool EASY_BMP_NAMESPACE::BMP::WriteToFile( const char* FileName )
{
 using namespace std;
 ...
 if( BitDepth == 16 )
 { dPaletteSize = ((int64_t)3*4); }
 
 double dTotalFileSize = ((int64_t)14 + 40) + dPaletteSize + dTotalPixelBytes;
 ...
 { bmih.biXPelsPerMeter = EBMP_DefaultXPelsPerMeter; }
 if( YPelsPerMeter )
 ...
 {  
  ebmpBYTE* Buffer;
  int BufferSize = (int) ( ((int64_t)Width*BitDepth)/8.0 );
...
bool EASY_BMP_NAMESPACE::BMP::WriteToStream(std::ostream& output)
{
    using namespace std;
    if (!EasyBMPcheckDataSize())
    {
    ...
    if ((output.flags() | std::ios::binary) == 0)
    {
        if (EasyBMPwarnings)
            cout << "EasyBMP Error: Output stream isn't binary!" << endl;
        return false;
    }

    //Nest the whole thing in try/catch;
    //    if IO fails, return false.
    try {
    ...
    #define EBMP_WRITE_VAR(var) \
        output.write((const char*)(&var), sizeof(decltype(var)))

    EBMP_WRITE_VAR(bmfh.bfType);
    EBMP_WRITE_VAR(bmfh.bfSize);
    EBMP_WRITE_VAR(bmfh.bfReserved1);
    EBMP_WRITE_VAR(bmfh.bfReserved2);
    EBMP_WRITE_VAR(bmfh.bfOffBits);
    ...
    EBMP_WRITE_VAR(bmih.biSize);
    EBMP_WRITE_VAR(bmih.biWidth);
    EBMP_WRITE_VAR(bmih.biHeight);
    EBMP_WRITE_VAR(bmih.biPlanes);
    EBMP_WRITE_VAR(bmih.biBitCount);
    EBMP_WRITE_VAR(bmih.biCompression);
    EBMP_WRITE_VAR(bmih.biSizeImage);
    EBMP_WRITE_VAR(bmih.biXPelsPerMeter);
    EBMP_WRITE_VAR(bmih.biYPelsPerMeter);
    EBMP_WRITE_VAR(bmih.biClrUsed);
    EBMP_WRITE_VAR(bmih.biClrImportant);
    ...
    if (BitDepth != 16)
    {
        ...
        while (j > -1)
        {
            ...
            if (BitDepth == 32)
            {
                Success = Write32bitRow(Buffer.data(), BufferSize, j);
            }
            if (BitDepth == 24)
            {
                Success = Write24bitRow(Buffer.data(), BufferSize, j);
            }
            if (BitDepth == 8)
            {
                Success = Write8bitRow(Buffer.data(), BufferSize, j);
            }
            if (BitDepth == 4)
            {
                Success = Write4bitRow(Buffer.data(), BufferSize, j);
            }
            if (BitDepth == 1)
            {
                Success = Write1bitRow(Buffer.data(), BufferSize, j);
            }
            if (Success)
            {
                output.write((const char*)Buffer.data(), (std::streamsize)BufferSize);
            }
            if (!Success)
            {
                if (EasyBMPwarnings)
    ...
    if (BitDepth == 16)
    {
        ...
        EBMP_WRITE_VAR(RedMask);
        EBMP_WRITE_VAR(ZeroWORD);

        if (IsBigEndian())
        {
            GreenMask = FlipWORD(GreenMask);
        }
        EBMP_WRITE_VAR(GreenMask);
        EBMP_WRITE_VAR(ZeroWORD);
...
bool EASY_BMP_NAMESPACE::BMP::ReadFromFile( const char* FileName )
{ 
 using namespace std;
 if( !EasyBMPcheckDataSize() )
 {
...
bool EASY_BMP_NAMESPACE::BMP::ReadFromStream(std::istream& input)
{
    using namespace std;
    if (!EasyBMPcheckDataSize())
    {
    ...
    if ((input.flags() | std::ios::binary) == 0)
    {
        if (EasyBMPwarnings)
            cout << "EasyBMP Error: Input stream isn't binary!" << endl;
        return false;
    }
    ...
    #define EBMP_READ_VAR(var) \
        input.read((char*)(&(var)), sizeof(decltype(var)))

    BMFH bmfh;

    EBMP_READ_VAR(bmfh.bfType);
    ...
    if (!IsBitmap)
    {
        if (EasyBMPwarnings)
        {
            cout << "EasyBMP Error: input stream " <<
                    "is not a Windows BMP file!" << endl;
        }
        return false;
    }

    EBMP_READ_VAR(bmfh.bfSize);
    EBMP_READ_VAR(bmfh.bfReserved1);
    EBMP_READ_VAR(bmfh.bfReserved2);
    EBMP_READ_VAR(bmfh.bfOffBits);
    ...
    EBMP_READ_VAR(bmih.biSize);
    EBMP_READ_VAR(bmih.biWidth);
    EBMP_READ_VAR(bmih.biHeight);
    EBMP_READ_VAR(bmih.biPlanes);
    EBMP_READ_VAR(bmih.biBitCount);
    EBMP_READ_VAR(bmih.biCompression);
    EBMP_READ_VAR(bmih.biSizeImage);
    EBMP_READ_VAR(bmih.biXPelsPerMeter);
    EBMP_READ_VAR(bmih.biYPelsPerMeter);
    EBMP_READ_VAR(bmih.biClrUsed);
    EBMP_READ_VAR(bmih.biClrImportant);
    ...
    if (bmih.biCompression == 1 || bmih.biCompression == 2)
    {
        if (EasyBMPwarnings)
        {
            cout << "EasyBMP Error: input stream is (RLE) compressed." << endl
                << "               EasyBMP does not support compression." << endl;
        }
        SetSize(1, 1);
        SetBitDepth(1);
    ...
    if (bmih.biCompression > 3)
    {
        if (EasyBMPwarnings)
        {
            cout << "EasyBMP Error: input stream is in an unsupported format."
                << endl
                << "               (bmih.biCompression = "
                << bmih.biCompression << ")" << endl
                << "               The file is probably an old OS2 bitmap, or corrupted."
        ...
        && TempBitDepth != 24 && TempBitDepth != 32)
    {
        if (EasyBMPwarnings)
        {
            cout << "EasyBMP Error: input stream has unrecognized bit depth." << endl;
        }
        SetSize(1, 1);
        SetBitDepth(1);
        return false;
    ...
    if (BitDepth < 16)
    {
        ...
        if (NumberOfColorsToRead < TellNumberOfColors())
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: file input stream has an underspecified" << endl
                    << "                 color table. The table will be padded with extra" << endl
                    << "                 white (255,255,255,0) entries." << endl;
            }
        }
    ...
    if (BytesToSkip > 0 && BitDepth != 16)
    {
        if (EasyBMPwarnings)
        {
            cout << "EasyBMP Warning: Extra meta data detected in input stream" << endl
                << "                 Data will be skipped." << endl;
        }
        vector<ebmpBYTE> TempSkipBYTE;
        TempSkipBYTE.resize(BytesToSkip);
        input.read((char*)TempSkipBYTE.data(), BytesToSkip);
    }
    ...
    if (BitDepth != 16)
    {
        ...
        while (j > -1)
        {
            input.read((char*)Buffer.data(), (std::streamsize)Buffer.size());
            bool Success = false;
            if (BitDepth == 1)
                Success = Read1bitRow(Buffer.data(), BufferSize, j);
            if (BitDepth == 4)
                Success = Read4bitRow(Buffer.data(), BufferSize, j);
            if (BitDepth == 8)
                Success = Read8bitRow(Buffer.data(), BufferSize, j);
            if (BitDepth == 24)
                Success = Read24bitRow(Buffer.data(), BufferSize, j);
            if (BitDepth == 32)
                Success = Read32bitRow(Buffer.data(), BufferSize, j);
            if (!Success)
            {
                if (EasyBMPwarnings)
                    cout << "EasyBMP Error: Could not read enough pixel data!" << endl;
    ...
    if (BitDepth == 16)
    {
        ...
        if (bmih.biCompression != 0)
        {
            ...
            EBMP_READ_VAR(RedMask);
            if (IsBigEndian())
            {
                RedMask = FlipWORD(RedMask);
            }
            EBMP_READ_VAR(TempMaskWORD);

            EBMP_READ_VAR(GreenMask);
            if (IsBigEndian())
            {
                GreenMask = FlipWORD(GreenMask);
            }
        ...
        if (BytesToSkip > 0)
        {
            if (EasyBMPwarnings)
            {
                cout << "EasyBMP Warning: Extra meta data detected in input stream "
                    << endl
                    << "                 Data will be skipped." << endl;
            }
            vector<ebmpBYTE> TempSkipBYTE;
...
bool EASY_BMP_NAMESPACE::BMP::CreateStandardColorTable( void )
{
 using namespace std;
 if( BitDepth != 1 && BitDepth != 4 && BitDepth != 8 )
 {
...
bool EASY_BMP_NAMESPACE::SafeFread( char* buffer, int size, int number, FILE* fp )
{
 using namespace std;
 int ItemsRead;
 if( feof(fp) )
...
void EASY_BMP_NAMESPACE::BMP::SetDPI( int HorizontalDPI, int VerticalDPI )
{
...
int EASY_BMP_NAMESPACE::BMP::TellVerticalDPI( void )
{
 if( !YPelsPerMeter )
...
int EASY_BMP_NAMESPACE::BMP::TellHorizontalDPI( void )
{
 if( !XPelsPerMeter )
 { XPelsPerMeter = EBMP_DefaultXPelsPerMeter; }
 return (int) ( XPelsPerMeter / (double) 39.37007874015748 );
}

/* These functions are defined in EasyBMP_VariousBMPutilities.h */

BMFH EASY_BMP_NAMESPACE::GetBMFH( const char* szFileNameIn )
{
 using namespace std;
 BMFH bmfh;
...
BMIH EASY_BMP_NAMESPACE::GetBMIH( const char* szFileNameIn )
{
 using namespace std;
 BMFH bmfh;
 BMIH bmih;
...
void EASY_BMP_NAMESPACE::DisplayBitmapInfo( const char* szFileNameIn )
{
 using namespace std;
 FILE* fp;
 fp = fopen( szFileNameIn,"rb");
...
int EASY_BMP_NAMESPACE::GetBitmapColorDepth( const char* szFileNameIn )
{
 BMIH bmih = GetBMIH( szFileNameIn );
 return (int) bmih.biBitCount;
}

void EASY_BMP_NAMESPACE::PixelToPixelCopy( BMP& From, int FromX, int FromY,
...
void EASY_BMP_NAMESPACE::PixelToPixelCopyTransparent( BMP& From, int FromX, int FromY,
                                  BMP& To, int ToX, int ToY,
...
void EASY_BMP_NAMESPACE::RangedPixelToPixelCopy( BMP& From, int FromL , int FromR, int FromB, int FromT,
                             BMP& To, int ToX, int ToY )
{
...
void EASY_BMP_NAMESPACE::RangedPixelToPixelCopyTransparent(
...
bool EASY_BMP_NAMESPACE::CreateGrayscaleColorTable( BMP& InputImage )
{
 using namespace std;
...
bool EASY_BMP_NAMESPACE::BMP::Read32bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*4 > BufferSize )
 { return false; }
 for( i=0 ; i < Width ; i++ )
 { memcpy( (char*) &(Pixels[i][Row]), (char*) Buffer+((int64_t)4*i), 4 ); }
 return true;
}

bool EASY_BMP_NAMESPACE::BMP::Read24bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*3 > BufferSize )
 { return false; }
 for( i=0 ; i < Width ; i++ )
 { memcpy( (char*) &(Pixels[i][Row]), Buffer+((int64_t)3*i), 3 ); }
 return true;
}

bool EASY_BMP_NAMESPACE::BMP::Read8bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int i;
 if( Width > BufferSize )
 { return false; }
...
bool EASY_BMP_NAMESPACE::BMP::Read4bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int Shifts[2] = {4  ,0 };
 int Masks[2]  = {240,15};
...
bool EASY_BMP_NAMESPACE::BMP::Read1bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
...
bool EASY_BMP_NAMESPACE::BMP::Write32bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*4 > BufferSize )
 { return false; }
 for( i=0 ; i < Width ; i++ )
 { memcpy( (char*) Buffer+((int64_t)4*i), (char*) &(Pixels[i][Row]), 4 ); }
 return true;
}

bool EASY_BMP_NAMESPACE::BMP::Write24bitRow( ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
 int i;
 if( Width*3 > BufferSize )
 { return false; }
 for( i=0 ; i < Width ; i++ )
 { memcpy( (char*) Buffer+((int64_t)3*i),  (char*) &(Pixels[i][Row]), 3 ); }
 return true;
}

bool EASY_BMP_NAMESPACE::BMP::Write8bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{
 int i;
 if( Width > BufferSize )
 { return false; }
...
bool EASY_BMP_NAMESPACE::BMP::Write4bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
...
bool EASY_BMP_NAMESPACE::BMP::Write1bitRow(  ebmpBYTE* Buffer, int BufferSize, int Row )
{ 
...
ebmpBYTE EASY_BMP_NAMESPACE::BMP::FindClosestColor( RGBApixel& input )
{
 using namespace std;
...
bool EASY_BMP_NAMESPACE::EasyBMPcheckDataSize( void )
{
 using namespace std;
...
bool EASY_BMP_NAMESPACE::Rescale( BMP& InputImage , char mode, int NewDimension )
{
 using namespace std;
 ...
 if( CapMode == 'P' )
 {
  NewWidth = (int) floor((int64_t)OldWidth * NewDimension / 100.0 );
  NewHeight = (int) floor((int64_t)OldHeight * NewDimension / 100.0 );
 }
 if( CapMode == 'F' )
 {
